...
function OnLoad() {
  ...
  // But wait! What good are the results we get if we can't see
  // them plotted on the map? So let's put them there!
  // First, we need to clear any markers already on the map
 ...
 // And second, we need is a search complete callback!
  searchControl.setSearchCompleteCallback(searcher , function() {
    ...
    map.setCenter(new google.maps.LatLng(center.lat, center.lng), zoom)
  });
 ...
 // Execute an initial search
  searchControl.execute('pizza near mountain view');
}
